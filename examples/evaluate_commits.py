#!/usr/bin/env python3
"""
Example script for evaluating bug fixes by comparing specific commits.

This script demonstrates how to use the Bug Fix Evaluator API to compare
bug fixes from engineer and AI developers using commit SHAs.
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate bug fixes by comparing specific commits"
    )
    parser.add_argument('--repo', required=True, help='URL to the git repository')
    parser.add_argument('--engineer', required=True, help='SHA of the engineer\'s bug fix commit')
    parser.add_argument('--ai', required=True, help='SHA of the AI\'s bug fix commit')
...
def main():
    ...
    try:
        ...
        print(f"Evaluating commits from repository: {args.repo}")
        print(f"Engineer commit: {args.engineer}")
        print(f"AI commit: {args.ai}")
